...
export default function TimeTrackingView({ onActiveEntryChange }: { onActiveEntryChange?: (entry: TimeEntry | null) => void }) {
  ...
  return (
    <section className="time-tracking-view">
        ...
        {loading ? <div className="settings-loading" role="status">Loading time tracking...</div> : <>
          ...
          {tab === "week" ? <>
            <form className="time-entry-form" onSubmit={(event) => { event.preventDefault(); void startEntry(); }}>
              ...
              <ProjectSelect projects={projects} selected={projectID} onChange={setProjectID} disabled={!!activeEntry || busy} />
      ...
      {editing && <div className="time-tracking-dialog" role="dialog" aria-modal="true" aria-label="Correct time entry"><form onSubmit={(event) => { event.preventDefault(); void saveEdit(); }}><h3>Correct time entry</h3><label>Task name<input value={editName} onChange={(event) => setEditName(event.target.value)} /></label><ProjectSelect projects={catalog?.projects ?? []} selected={editProjectID} onChange={setEditProjectID} /><TagMultiSelect tags={catalog?.tags ?? []} selected={editTagIDs} onChange={setEditTagIDs} /><label>Started<input type="datetime-local" value={editStartedAt} onChange={(event) => setEditStartedAt(event.target.value)} /></label><label>Ended<input type="datetime-local" value={editEndedAt} onChange={(event) => setEditEndedAt(event.target.value)} /></label><div className="settings-actions"><button type="button" className="secondary-button" onClick={() => setEditing(null)}>Cancel</button><button className="primary-button" disabled={busy}>Save correction</button></div></form></div>}
